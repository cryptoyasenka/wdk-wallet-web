...
| `connect-src` | `'self'` + default RPC origins + `https://api.coingecko.com` + `NEXT_PUBLIC_ETHEREUM_RPC_URLS` origins + `NEXT_PUBLIC_CONNECT_SRC_ORIGINS` origins + conditional `wss:` | Pins network egress. The allowed origins come from one shared module, [`src/lib/cspAllowlist.ts`](../apps/next/src/lib/cspAllowlist.ts): `middleware.ts` builds `connect-src` from it and the Data Sources UI reads the same list to warn the user (see honest limit below), so the two cannot drift. The default RPC origins mirror `chains/index.ts` public lists; that link is itself drift-guarded by [`test/cspAllowlist.test.ts`](../apps/next/test/cspAllowlist.test.ts) (it runs in Node and can import both, where the Edge bundle cannot). Two deploy-env sources are folded in: `NEXT_PUBLIC_ETHEREUM_RPC_URLS` (the same var `engine.ts` reads for the Ethereum RPC chain) and `NEXT_PUBLIC_CONNECT_SRC_ORIGINS`, a dedicated knob that allow-lists an origin for the CSP *only*, so a self-hoster can permit a custom indexer/price/RPC origin without it doubling as an Ethereum RPC. CoinGecko is the disclosed price oracle. Out of the box, `wss:` is allowed wholesale because the Bitcoin Electrum-WS endpoint is operator-supplied via `NEXT_PUBLIC_BTC_ELECTRUM_WS_URL` and has no universal public origin to pin. Once a hardened deploy lists at least one explicit `wss://` Electrum origin in `NEXT_PUBLIC_CONNECT_SRC_ORIGINS`, the emitted CSP drops the wholesale `wss:` scheme and permits only the pinned secure-WebSocket origin(s). |
...
- **The browser cannot open raw Electrum TCP**, so BTC requires an
  Electrum-over-WebSocket endpoint. The default CSP allows `wss:` for this
  operator-supplied endpoint, and production deploys can pin the exact
  `wss://` origin via `NEXT_PUBLIC_CONNECT_SRC_ORIGINS`.
