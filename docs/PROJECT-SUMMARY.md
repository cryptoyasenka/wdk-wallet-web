...
- **Walkthrough video (~90 s, silent):** [`walkthrough.mp4`](walkthrough.mp4) - create → back up → portfolio → receive → send form
...
each, is in [`Verification checklist`](VERIFICATION-CHECKLIST.md).
...
- **Not for production use with real funds yet** - the web has no exact equivalent of
  ...
  and residual XSS risk are stated plainly in [`SECURITY.md`](SECURITY.md).
