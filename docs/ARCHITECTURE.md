...
UI shell brings First Load to ≈240 kB. See "First Load JS" below.

**Language scope:** `apps/svelte` is **English-only by design**. The localized
(en/ru/uk) reference wallet is `apps/next`; the Svelte app exists to prove the
shared core is framework-agnostic, not to duplicate the Next app's full UI
feature set. Adding Svelte localization would not strengthen the portability
claim.
...
BTC. Current First Load: Next ≈ 240 kB, Svelte main ≈ 226 kB; both grew from
...
- `crypto.worker.ts`, the Dedicated Worker entry: a `WdkCoreAdapter`, signer/
...
`@tetherto/*` (Next First Load JS ≈ 240 kB; no `@tetherto/*` in it before or
...
unit-tested in `packages/wallet-core/test/vault.test.ts` (part of the 94-green wallet-core suite). The
