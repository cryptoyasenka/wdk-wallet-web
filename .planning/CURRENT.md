...
**FINAL PRE-SUBMISSION AUDIT (2026-05-29):** Full adversarial pass (crypto/host/anti-phishing/network/UI/supply-chain/live user-flow). Verdict: code is exceptionally clean + honest; crypto core strong (worker seed isolation, PBKDF2-600k/HKDF/AES-GCM, WebAuthn PRF all correct); zero XSS sinks / console / type-escapes / secret-logging; i18n complete (every key en+ru+uk, every used key defined). Four fixes shipped to main, all green (verify 88+97+16=201 tests, smoke 6/6, a11y 0-violations/8-screens, audit 1 LOW=documented elliptic):
...
NEXT (Yana's call): build a **fee speed selector** (slow/normal/fast) as a v2 feature — she picked it over the minimal warn/reserve. Fee is already shown at confirm (page.tsx:1425). Facts: EVM accepts maxFeePerGas/maxPriorityFeePerGas, BTC accepts feeRate+confirmationTarget, Solana has no SDK priority-fee field (show without tiers, honest). It's a >1h cross-cutting change (thread fee opts through TxIntent → worker-protocol → wdk-core → engine → page.tsx + per-chain tier estimation + tests); per п.2, write a short PLAN first, then implement (likely a fresh session). It also subsumes the Max-on-native gas footgun (reserve the chosen fee).
