...
**FINAL PRE-SUBMISSION AUDIT (2026-05-29):** Full adversarial pass (crypto/host/anti-phishing/network/UI/supply-chain/live user-flow). Verdict: code is exceptionally clean + honest; crypto core strong (worker seed isolation, PBKDF2-600k/HKDF/AES-GCM, WebAuthn PRF all correct); zero XSS sinks / console / type-escapes / secret-logging; i18n complete (every key en+ru+uk, every used key defined). Three fixes shipped to main, all green (verify 84+97+16=197 tests, smoke 6/6, a11y 0-violations/8-screens, audit 1 LOW=documented elliptic):
...
STILL OPEN (not fixed this pass): (a) **Max native-coin gas footgun** — "Send entire balance" on a NATIVE asset (ETH/BTC/SOL/POL/XPL) reserves no fee → tx fails; needs design (reserve estimated fee vs warn). Token Max is fine. (b) LOW robustness: `wdk/worker-proxy.ts` has no `worker.onerror`/`onmessageerror` → a worker crash hangs pending promises.
