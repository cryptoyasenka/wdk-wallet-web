...
- **AUTO-DEPLOY LIVE (2026-05-27)**: pushes to `main` now auto-deploy. Yana granted the Railway GitHub App access to the repo (Settings → Applications → Railway → Configure → Repository access), then `deploymentTriggerCreate` succeeded → repoTrigger `3cc53dda-8d66-4f18-bfe3-62aded0f10dc` (branch main, repo cryptoyasenka/wdk-wallet-web) + serviceInstance source.repo set. Manual `railway up` (token ~/.config/railway-token.env) still works as a fallback if a push-deploy ever needs forcing.
