...
- **DEPLOY DONE**: merged main shipped via `railway up` (deployment 6f8deb2f, SUCCESS, Solana confirmed live). Reminder for future pushes: Railway is NOT wired to GitHub auto-deploy — every green push needs a manual `railway up` (token ~/.config/railway-token.env) to reach the live demo.
