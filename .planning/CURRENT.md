...
**Last touched:** 2026-05-27 (independent re-audit fixes — ALL 3 done)
**Status:** LIVE on Railway + /cso clean. All 3 independent-audit findings fixed, green, committed. Findings 3 (watch-only label clear, 40d4795) + 1 (indexer historyProvider wiring, eb7fe41) + 2 (CSP/Data Sources dedup + UI warning, Variant B) DONE. URL: https://wdk-wallet-web-production.up.railway.app. Green: verify (lint+typecheck+build + 80 wallet-core + ~86 next + 13 svelte tests) + smoke (6 checks under live nonce CSP) both pass.
...
## ⚠️ RAILWAY DEPLOY STALE (2026-05-27) — needs Yana
GitHub perfect: clean tree, HEAD==origin/main, CI success for f9c4286. BUT live https://wdk-wallet-web-production.up.railway.app runs OLD code. Proof: live page chunk has control string "Save data sources" but NONE of f9c4286's new strings ("will block requests"/"allow-list"/"blockedOrigins"). Railway didn't rebuild after recent pushes (auto-deploy not firing). Railway CLI logged out (OAuth invalid_grant), no token in env → I can't redeploy myself. Yana: `! railway login` then `railway up`/`railway redeploy`, OR enable GitHub auto-deploy on the service. Verify-fresh one-liner: `BASE=https://wdk-wallet-web-production.up.railway.app; P=$(curl -sS $BASE/ | grep -oE '/_next/static/chunks/app/page-[^"]+\.js' | head -1); curl -sS --compressed "$BASE$P" | grep -c "will block requests"` → 1=fresh.
